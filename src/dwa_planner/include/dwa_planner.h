...
private:
    ...
    void getTarget(const geometry_msgs::PointStamped &target);
    void getHeading(const geometry_msgs::PointStamped &heading);
    void resetTime(const std_msgs::Float32 &time);
    void getTurningFlag(const std_msgs::Int8 &dir);
    void getState(const std_msgs::Int8 &data);
    void getLotType(const std_msgs::Int8 &data);
    /**
     * @brief 获取坐标(x,y)处的数据
     */
    inline int getMapData(int x, int y)
    {
        // dwa的地图坐标系与车辆固结，设置为前y右x， 但发过来的所有量都是前x左y
        return int(map_->data[y + (-x) * map_->info.width]);
    }
    ...
    ros::Publisher current_target_point_pub;
    ros::Publisher current_target_heading_pub;
    ros::Publisher restart_drive_pub;
    ...
    ros::Subscriber dwa_state_sub;
    ros::Subscriber lot_type_sub;
