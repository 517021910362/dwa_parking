...
// std::pair<int, float> SCManager::relocalize(pcl::PointCloud<SCPointType> & _match_pcloud)
// {
//     int match_id = -1;
//     Eigen::MatrixXd curr_desc = makeScancontext(_match_pcloud); // v1 
...
//         cout << "[Loop found] id: " << nn_idx << endl;
...
//     std::pair<int, float> result {match_id, yaw_diff_rad};
...
void SCManager::treereconstruction()
{
...
std::pair<int, float> SCManager::relocalize( pcl::PointCloud<SCPointType> & _scan_down )
{
    Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1 
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( sc );
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( sc );
    ...
    if( min_dist < SC_DIST_THRES )
    {
        ...
        // std::cout.precision(3); 
        cout << "[Loop found] Nearest distance: " << min_dist << " with " << nn_idx << "." << endl;
        cout << "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    }
    else
    {
        std::cout.precision(3); 
        cout << "[Not loop] Nearest distance: " << min_dist << " with " << nn_idx << "." << endl;
        cout << "[Not loop] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    }
